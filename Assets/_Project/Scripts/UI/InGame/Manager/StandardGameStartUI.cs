...
/// <summary>
/// 게임 시작에 관련한 UI에 대한 스크립트입니다. 
/// 필드 편성 화면 / 웨이브 진행(배치 변경 및 Hold 시 information) 
/// </summary>
public class StandardGameStartUI : MonoBehaviour
{
    #region ◇ Parameters ◇
    static StandardGameStartUI instance;
        ...
        switch (touch.phase)
        {
            ...
            case TouchPhase.Moved:// 터치가 고정되거나 약간 움직이는 경우
                if (isPressing)
                {
                    ...
                    {
                        Debug.Log("[Standard Game Start UI]터치가 움직였습니다. Hold가 취소되었습니다.");
                        isPressing = false;
                    }
    ...
    {
        Debug.Log("[Standard Game Start UI]뒤로가기 버튼을 눌렸습니다. ");
    }
    ...
    {
        Debug.Log("[Standard Game Start UI]게임에 입장하셨습니다. ");
    ...
    void CreateUnit(uint num) //Unit 생성 
    {
        ...
        else
        {
            ...
            Debug.Log("[Standard Game Start UI]유닛 배치 최대입니다.");
        }
    }
        ...
        for (int i = 0; i < tileInfo.Length; i++)
        {
            if (tileInfo[i].Unit != null)
            {
                if (tileInfo[i].Unit.Status == UnidadManager.Instance.GetStatus(num))
                {
                    ...
                    Debug.Log("[Standard Game Start UI]해당 유닛을 제거하였습니다.");
                    break;
                }
            }
        }
